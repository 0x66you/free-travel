...
def booking_plan2(place,cidate,codate,human,choose,lastname,firstname,email,city,address,phonenum,cardnumber1,cardnumber2,cardnumber3,cardnumber4,ccmonth,ccyear,cvc):
    ...
    time.sleep(5)
    browser.find_elements_by_class_name('bbdb949247')[1].click()
    time.sleep(3)
    ...
    for e in extraneed:
        ...
        if var in ["飯店","汽車旅館","民宿"] :
            ...
            break
    time.sleep(6)
    hotel_Star = browser.find_elements_by_class_name('e278108e40')
    for hh in hotel_Star:
        var = hh.text
        if var in ["4 星級"] :
            hh.click()
            break
    ...
    while True:
        ...
        if rooms > x :
            ...
            break
    Select(browser.find_elements_by_class_name('hprt-nos-select')[count]).select_by_value("1")
